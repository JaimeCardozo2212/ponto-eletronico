...
def create_allocation(time_entry_id: int, project_id: int, hours: float, notes: str = "") -> int:
    conn = get_connection()
    cursor = conn.execute(
        "INSERT INTO project_allocations (time_entry_id, project_id, hours, notes) VALUES (?, ?, ?, ?)",
        (time_entry_id, project_id, hours, notes),
    )
    conn.commit()
    row_id = cursor.lastrowid
    conn.close()
